from time import sleep
from random import randint
import sqlite3


def exit() -> None:
    print("Всё, игра закончена. Программа выключится через 8 секунд.")
    for downtime in range(8, 0, -1):
        if downtime in {4, 3, 2}:
            print(f"Осталось {downtime} секунды")
        elif downtime == 1:
            print(f"Осталась 1 секунда")
        else:
            print(f"Осталось {downtime} секунд")
        sleep(1)
    quit()


def input(text='') -> str:
    answer = input(text)
    if a == '!':
        exit()
    answer = answer.lower()
    return answer


def show_cities_rules() -> None:
    text = "Правила игры в 'Города России':\n\
Все участники по очереди называют города России.\n\
Название должно начинаться на букву предыдущего названного города.\n"
    print(text)
    sleep(8)


def show_numbers_rules() -> None:
    text = "Правила игры в 'Числа':\n\
Назовите любое целое число от 1 до 100. Нельзя называть уже названные \n\
числа, следовательно, в одной игре не может быть больше 50 конов \n\
У кого число больше, тот и выиграл кон.\n\
При этом, *совпадение чисел допускается* ."
    print(text)
    sleep(8)


def show_game_name(gametype: str) -> None:
    print(f"Вы выбрали пункт '{gametype}'")


def show_game_started(gametype: str) -> None:
    print(f'Игра в {gametype} началась.')


def ask_one_two(request: str) -> int:
    answer = input(request)
    while answer not in {"1", "2"}:
        print("Ввод не распознан. Проверьте, что число было введено без кавычек.")
	print("Принимаются только числа 1 и 2.")
        answer = input(request)
    if answer == '1':
        return 1
    return 2


if __name__ == "__main__":
    hello = 'Приветствую!'
    print(hello)
    game_choice_text = 'Выберите один из следующих пунктов, чтобы продолжить:\n\
- "Города России". Введите "1", чтобызапустить.\n\
- "Числа". Введите "2", чтобы запустить.\n\
Ваш выбор: '
    game_choice = ask_one_two(game_choice_text)
    if game_choice == 1:
        m_city_bot_lost = "Я проиграл и не знаю города на такую букву"
        game_name = 'Города России'
        abc = {
            "а": [
                "Абакан",
                "Азов",
                "Александров",
                "Алексин",
                "Альметьевск",
                "Анапа",
                "Ангарск",
                "Анжеро-Судженск",
                "Апатиты",
                "Арзамас",
                "Армавир",
                "Арсеньев",
                "Артем",
                "Архангельск",
                "Асбест",
                "Астрахань",
                "Ачинск",
                "Алушта",
                "Абаза",
                "Абинск",
                "Агидель",
                "Агрыз",
                "Адыгейск",
                "Ак-Довурак",
                "Алагир",
                "Алапаевск",
                "Алдан",
                "Алзамай",
                "Армянск",
                "Ахтубинск",
                "Аткарск",
                "Арск",
            ],
            "б": [
                "Бабаево",
                "Барабинск",
                "Балашов",
                "Батайск",
                "Балабаново",
                "Балахна",
                "Балашиха",
                "БрянскБалтийск",
                "Березники",
                "Бологое,Бежецк",
                "Белая Калитва",
                "Беслан",
                "Благовещенск",
                "Бердск",
                "Белоярский",
                "Братск",
                "Бобров",
                "Белгород",
                "Березовский",
                "Баксан",
                "Боровичи",
                "Белебей",
                "Бирск",
                "Барнаул",
                "Большой Камень",
                "Белово",
                "Благовещенск",
                "Биробиджан",
                "Бор",
                "Белогорск",
                "Благодарный",
                "Богородицк",
                "Борзя",
                "Белорецк",
                "Богданович",
                "Борисоглебск",
                "Бийск",
                "Белореченск",
                "Бугульма",
                "Буденновск",
                "Буй",
                "Бутурлиновка",
                "Бугуруслан",
                "Бузулук",
                "Буйнакск",
                "Бавлы",
                "Белокуриха",
                "Буинск",
                "Белозерск",
                "Баймак",
                "Бикин",
                "Болотное",
                "Багратионовск",
                "Бакал",
                "Боготол",
                "Бородино",
                "Богучар",
                "Балаково",
                "Бодайбо",
                "Бронницы",
                "Беломорск",
                "Барыш",
                "Бокситогорск",
                "Боровск",
                "Бирюч",
                "Белый",
                "Белоусово",
                "Болхов",
                "Болохово",
                "Белев",
                "Белая Холуница",
                "Богородск",
                "Болгар",
                "Белинский",
                "Бахчисарай",
                "Белогорск",
                "Балей",
                "Байкальск",
                "Бабушкин",
                "Билибино",
                "Бирюсинск",
                "Белоозерский",
                "Балтийск",
                "Бологое",
            ],
            "в": [
                "Валдай",
                "Валуйки," "Велиж",
                "Великие Луки",
                "Великий Новгород",
                "Великий Устюг",
                "Вельск",
                "Венёв",
                "Верещагино",
                "Верея",
                "Верхнеуральск",
                "Верхний Тагил",
                "Верхний Уфалей",
                "Верхняя Пышма",
                "Верхняя Салда",
                "Верхняя Тура",
                "Верхотурье",
                "Верхоянск",
                "Весьегонск",
                "Ветлуга",
                "Видное",
                "Вилюйск",
                "Вилючинск",
                "Вихоревка",
                "Вичуга",
                "Владивосток",
                "Владикавказ",
                "Владимир",
                "Волгоград",
                "Волгодонск",
                "Волгореченск",
                "Волжск",
                "Волжский",
                "Вологда",
                "Володарск",
                "Волоколамск",
                "Волосово",
                "Волхов",
                "Волчанск",
                "Вольск",
                "Воркута",
                "Воронеж",
                "Ворсма",
                "Воскресенск",
                "Воткинск",
                "Всеволожск",
                "Вуктыл",
                "Выборг",
                "Выкса",
                "Высоковск",
                "Высоцк",
                "Вытегра",
                "Вышний Волочёк",
                "Вяземский",
                "Вязники",
                "Вязьма",
                "Вятские Поляны",
            ],
            "г": [
                "Гагарин",
                "Губаха",
                "Гай",
                "Голицыно",
                "Гатчина",
                "Губкинский",
                "Галич",
                "Гуково",
                "Глазов",
                "Губкин",
                "Гаврилов-Ям",
                "Горно-Алтайск",
                "Городец",
                "Гулькевичи",
                "Гороховец",
                "Грозный",
                "Горячий Ключ",
                "Гусь-Хрустальный",
                "Геленджик",
                "Гудермес",
                "Грязи",
                "Гурьевск",
                "Георгиевск\tГусев",
                "Грязовец",
                "Гусиноозерск",
                "Гаджиево",
                "Гдов",
                "Гвардейск",
                "Гурьевск",
                "Гаврилов Посад",
                "Грайворон",
                "Горбатов",
                "Городище",
                "Горнозаводск",
                "Гремячинск",
                "Горняк",
                "Городовиковск",
            ],
            "д": [
                "Дальнегорск",
                "Дубна",
                "Дальнереченск",
                "Данков",
                "Дедовск",
                "Дюртюли",
                "Дагестанские Огни",
                "Димитровград",
                "Дзержинский",
                "Дзержинск",
                "Дербент",
                "Дмитров",
                "Дивногорск",
                "Домодедово",
                "Десногорск",
                "Долгопрудный",
                "Добрянка",
                "Донецк",
                "Дудинка",
                "Дятьково",
                "Донской",
                "Давлеканово",
                "Дубовка",
                "Данилов",
                "Дрезна",
                "Демидов",
                "Дно",
                "Дмитровск",
                "Дмитриев",
                "Дорогобуж",
                "Духовщина",
                "Джанкой",
                "Далматово",
                "Долинск",
                "Дигора",
                "Дегтярск",
            ],
            "е": [
                "Егорьевск",
                "Ершов",
                "Ефремов",
                "Ейск",
                "Елизово",
                "Ессентуки",
                "Елабуга",
                "Еманжелинск",
                "Екатеринбург",
                "Енисейск",
                "Евпатория",
                "Ельня",
                "Елец",
                "Ермолино",
                "Емва",
            ],
            "ё": None,
            "ж": [
                "Железногорск",
                "Железногорск",
                "Железногорск-Илимский",
                "Железнодорожный",
                "Жигулевск",
                "Железноводск",
                "Жердевка",
                "Жуковка",
                "Жирновск",
                "Жуковский",
                "Жиздра",
                "Жуков",
            ],
            "з": [
                "Заволжье",
                "Златоуст",
                "Заводоуковск",
                "Заполярный",
                "Заинск",
                "Знаменск",
                "Зарайск",
                "Заволжск",
                "Заречный",
                "Западная Двина",
                "Зверево",
                "Звенигород",
                "Заринск",
                "Заозерск",
                "Зеленогорск",
                "Звенигово",
                "Зеленодольск",
                "Зуевка",
                "Зерноград",
                "Зеленоградск",
                "Зеленокумск",
                "Злынка",
                "Змеиногорск",
                "Заречный",
                "Зима",
                "Зубцов",
                "Зея",
                "Задонск",
                "Заозёрный",
                "Закаменск",
                "Зеленогорск",
                "Зеленоград",
                "Завитинск",
            ],
            "и": [
                "Ивантеевка",
                "Ивангород",
                "Игарка",
                "Ивдель",
                "Иваново",
                "Ижевск",
                "Изобильный",
                "Инза",
                "Инта",
                "Ипатово",
                "Избербаш",
                "Ирбит",
                "Исилькуль",
                "Иланский",
                "Истра",
                "Искитим",
                "Иркутск",
                "Ишим",
                "Инсар",
                "Иннополис",
                "Инкерман",
                "Ишимбай",
            ],
            "й": ["Йошкар-Ола"],
            "к": [
                "Кадников",
                "Казань",
                "Калач",
                "Калач-на-Дону",
                "Калачинск",
                "Калининград",
                "Калининск",
                "Калтан",
                "Калуга",
                "Калязин",
                "Камбарка",
                "Каменка",
                "Каменногорск",
                "Каменск-Уральский",
                "Каменск-Шахтинский",
                "Камень-на-Оби",
                "Камешково",
                "Камызяк",
                "Камышин",
                "Камышлов",
                "Канаш",
                "Кандалакша",
                "Канск",
                "Карабаново",
                "Карабаш",
                "Карабулак",
                "Карасук",
                "Карачаевск",
                "Карачев",
                "Каргат",
                "Каргополь",
                "Карпинск",
                "Касимов",
                "Касли",
                "Каспийск",
                "Катав-Ивановск",
                "Катайск",
                "Качканар",
                "Керчь",
                "Кизел",
                "Кизляр",
                "Кимовск",
                "Кимры",
                "Кингисепп",
                "Кинель",
                "Кинешма",
                "Киреевск",
                "Киренск",
                "Киржач",
                "Кириллов",
                "Кириши",
                "Киров",
                "Кировград",
                "Кирово-Чепецк",
                "Кировск",
                "Кирс",
                "Кирсанов",
                "Киселевск",
                "Кисловодск",
                "Климовск",
                "Клин",
                "Ковдор",
                "Ковров",
                "Когалым",
                "Кодинск",
                "Козельск",
                "Козьмодемьянск",
                "Кола",
                "Кологрив",
                "Коломна",
                "Колпашево",
                "Колпино",
                "Кольчугино",
                "Коммунар",
                "Комсомольск",
                "Комсомольск-на-Амуре",
                "Конаково",
                "Кондопога",
                "Кондрово",
                "Константиновск",
                "Копейск",
                "Кораблино",
                "Кореновск",
                "Королев",
                "Корсаков",
                "Коряжма",
                "Костомукша",
                "Кострома",
                "Котельники",
                "Котлас",
                "Котово",
                "Котовск",
                "Кохма",
                "Красноармейск",
                "Красновишерск",
                "Красногорск",
                "Красное Село",
                "Краснозаводск",
                "Краснознаменск",
                "Краснокаменск",
                "Краснокамск",
                "Красноперекопск",
                "Краснослободск",
                "Краснотурьинск",
                "Красноуральск",
                "Красноуфимск",
                "Красноярск",
                "Красный",
                "Кременки",
                "Кронштадт",
                "Кропоткин",
                "Крымск",
                "Кстово",
                "Кубинка",
                "Кувандык",
                "Кудымкар",
                "Кузнецк",
                "Куйбышев",
                "Кукмор",
                "Кулебаки",
                "Кумертау",
                "Кунгур",
                "Купино",
                "Курган",
                "Курганинск",
                "Курильск",
                "Курлово",
                "Куровское",
                "Курск",
                "Куртамыш",
                "Курчалой",
                "Курчатов",
                "Куса",
                "Кушва",
                "Кызыл",
                "Кыштым",
                "Кяхта",
            ],
            "л": [
                "Лабинск",
                "Лабытнанги",
                "Лагань",
                "Лакинск",
                "Лангепас",
                "Лахденпохья",
                "Лебедянь",
                "Лениногорск",
                "Ленинск",
                "Ленинск-Кузнецкий",
                "Ленск",
                "Лермонтов",
                "Лесной",
                "Лесозаводск",
                "Лесосибирск",
                "Ливны",
                "Ликино-Дулево",
                "Липецк",
                "Липки",
                "Лиски",
                "Лихославль",
                "Лобня",
                "Лодейное Поле",
                "Ломоносов",
                "Лосино-Петровский",
                "Луга",
                "Луза",
                "Лукоянов",
                "Луховицы",
                "Лысьва",
                "Льгов",
                "Любань",
                "Люберцы",
                "Лянтор",
            ],
            "м": [
                "Магадан",
                "Магас",
                "Магнитогорск",
                "Майкоп",
                "Макаров",
                "Макарьев",
                "Малая Вишера",
                "Малгобек",
                "Малмыж",
                "Малоархангельск",
                "Малоярославец",
                "Мамадыш",
                "Мамоново",
                "Мантурово",
                "Мариинск",
                "Мариинский Посад",
                "Маркс",
                "Махачкала",
                "Мглин",
                "Мегион",
                "Медвежьегорск",
                "Медногорск",
                "Медынь",
                "Межгорье",
                "Междуреченск",
                "Мезень",
                "Меленки",
                "Мелеуз",
                "Менделеевск",
                "Мензелинск",
                "Мещовск",
                "Миасс",
                "Микунь",
                "Миллерово",
                "Минеральные Воды",
                "Минусинск",
                "Миньяр",
                "Михайлова",
                "Михайловск",
                "Мичуринск",
                "Могоча",
                "Можайск",
                "Можга",
                "Моздок",
                "Мончегорск",
                "Морозовск",
                "Моршанск",
                "Мосальск",
                "Москва",
                "Мураши",
                "Мурино",
                "Мурманск",
                "Муром",
                "Мценск",
                "Мыски",
                "Мытищи",
                "Мышкин",
            ],
            "н": [
                "Набережные Челны",
                "Навашино",
                "Наволоки",
                "Надым",
                "Назарово",
                "Назрань",
                "Называевск",
                "Нальчик",
                "Нариманов",
                "Наро-Фоминск",
                "Нарткала",
                "Нарьян-Мар",
                "Находка",
                "Невель",
                "Невельск",
                "Невинномысск",
                "Невьянск",
                "Нелидово",
                "Неман",
                "Нерехта",
                "Нерчинск",
                "Нерюнгри",
                "Нестеров",
                "Нефтегорск",
                "Нефтекамск",
                "Нефтекумск",
                "Нефтеюганск",
                "Нея ",
                "Нижневартовск",
                "Нижнекамск",
                "Нижнеудинск",
                "Нижний Новгород",
                "Нижний Тагил",
                "Нижняя Салда",
                "Нижняя Тура",
                "Николаевск",
                "Николаевск-на-Амуре",
                "Никольск",
                "Никольское",
                "Новая Ладога",
                "Новоалександровск",
                "Новоалтайск",
                "Новоаннинский",
                "Нововоронеж",
                "Новодвинск",
                "Новозыбков",
                "Новокубанск",
                "Новокузнецк",
                "Новокуйбышевск",
                "Новомичуринск",
                "Новомосковск",
                "Новопавловск",
                "Новоржев",
                "Новороссийск",
                "Новосибирск",
                "Новосиль",
                "Новосокольники",
                "Новотроицк",
                "Новоузенск",
                "Новоульяновск",
                "Новоуральск",
                "Новохопёрск",
                "Новочебоксарск",
                "Новочеркасск",
                "Новошахтинск",
                "Новый Оскол",
                "Новый Уренгой",
                "Ногинск",
                "Нолинск",
                "Норильск",
                "Ноябрьск",
                "Нурлат",
                "Нюрба",
                "Нягань",
                "Нязепетровск",
                "Няндома",
            ],
            "о": [
                "Обнинск",
                "Обоянь",
                "Обь",
                "Одинцово",
                "Ожерелье",
                "Озёрск",
                "Озёры",
                "Октябрьск",
                "Октябрьский",
                "Окуловка",
                "Олёкминск",
                "Оленегорск",
                "Олонец",
                "Омск",
                "Омутнинск",
                "Онега",
                "Орёл",
                "Оренбург",
                "Орехово-Зуево",
                "Орлов",
                "Орск",
                "Оса",
                "Осинники",
                "Осташков",
                "Остров",
                "Островной",
                "Острогожск",
                "Отрадное",
                "Отрадный",
                "Оха",
                "Оханск",
                "Очёр",
            ],
            "п": [
                "Павловск",
                "Павловский Посад",
                "Партизанск",
                "Певек",
                "Пенза",
                "Первомайск",
                "Первоуральск",
                "Перевоз",
                "Переславль-Залесский",
                "Пермь",
                "Петергоф",
                "Петров Вал",
                "Петровск" "Петровск-Забайкальский",
                "Петрозаводск",
                "Петропавловск-Камчатский",
                "Петушки",
                "Печора",
                "Печоры",
                "Пикалево,",
                "Пионерский",
                "Питкяранта",
                "Плавск",
                "Пласт",
                "Плес",
                "Подольск",
                "Подпорожье",
                "Покачи",
                "Покров",
                "Покровск",
                "Полевской",
                "Полесск",
                "Полысаево",
                "Полярный",
                "Порхов",
                "Почеп",
                "Починок",
                "Пошехонье",
                "Правдинск",
                "Приволжск",
                "Приморск",
                "Приморско-Ахтарск",
                "Приозерск",
                "Прокопьевск",
                "Пролетарск",
                "Протвино",
                "Прохладный",
                "Псков",
                "Пугачев",
                "Пудож",
                "Пучеж",
                "Пушкин",
                "Пущино",
                "Пыть-Ях",
                "Пятигорск",
            ],
            "р": [
                "Райчихинск",
                "Раменское",
                "Реж",
                "Реутов",
                "Ржев",
                "Родники",
                "Рославль",
                "Россошь",
                "Ростов-на-Дону",
                "Ростов",
                "Рошаль",
                "Ртищево",
                "Рубцовск",
                "Рыбинск",
                "Рыльск",
                "Ряжск",
                "Рязань",
            ],
            "с": [
                "Салават",
                "Салаир",
                "Салехард",
                "Самара",
                "Санкт-Петербург",
                "Саранск",
                "Сарапул",
                "Саратов",
                "Севастополь",
                "Северо-Курильск",
                "Северобайкальск",
                "Северодвинск",
                "Североморск",
                "Североуральск",
                "Сергиев Посад",
                "Сердобск",
                "Серов",
                "Серпухов",
                "Сертолово",
                "Сестрорецк",
                "Сибай",
                "Сим",
                "Симферополь",
                "Сковородино",
                "Скопин",
                "Славгород",
                "Славск",
                "Славянск-на-Кубани",
                "Сланцы",
                "Слободской",
                "Слюдянка",
                "Смоленск",
                "Снежногорск",
                "Собинка",
                "Советск",
                "Сокол",
                "Солигалич",
                "Соликамск",
                "Солнечногорск",
                "Соль-Илецк",
                "Сольвычегодск",
                "Сольцы",
                "Сорочинск",
                "Сорск",
                "Сортавала",
                "Сосновый Бор",
                "Сосногорск",
                "Сочи",
                "Спасск",
                "Спасск-Дальний",
                "Спасск-Рязанский",
                "Среднеколымск",
                "Среднеуральск",
                "Сретенск",
                "Ставрополь",
                "Старая Русса",
                "Старица",
                "Стародуб",
                "Старый Крым",
                "Старый Оскол",
                "Стерлитамак",
                "Стрежевой",
                "Строитель",
                "Суворов",
                "Судак",
                "Суджа",
                "Судогда",
                "Суздаль",
                "Сунжа",
                "Суоярви",
                "Сураж",
                "Сургут",
                "Сухиничи",
                "Сухой Лог",
                "Сызрань",
                "Сыктывкар",
            ],
            "т": [
                "Тавда",
                "Таганрог",
                "Тайга",
                "Тайшет",
                "Талдом",
                "Талица",
                "Тамбов",
                "Тара",
                "Тарко-Сале",
                "Таруса",
                "Татарск",
                "Таштагол",
                "Тверь",
                "Теберда",
                "Темников",
                "Темрюк",
                "Терек",
                "Тетюши",
                "Тимашевск",
                "Тихвин",
                "Тихорецк",
                "Тобольск",
                "Тогучин",
                "Тольятти",
                "Томари",
                "Томмот",
                "Томск",
                "Топки",
                "Торжок",
                "Торопец",
                "Тосно",
                "Тотьма",
                "Трехгорный",
                "Троицк",
                "Трубчевск",
                "Туапсе",
                "Туймазы",
                "Тула",
                "Тулун",
                "Туран",
                "Туринск",
                "Тутаев",
                "Тында",
                "Тырныауз",
                "Тюкалинск",
                "Тюмень",
            ],
            "у": [
                "Уварово",
                "Углегорск",
                "Углич",
                "Удачный",
                "Удомля",
                "Ужур",
                "Узловая",
                "Улан-Удэ",
                "Ульяновск",
                "Унеча",
                "Урай",
                "Урень",
                "Уржум",
                "Урус-Мартан",
                "Урюпинск",
                "Усинск",
                "Усмань",
                "Усолье-Сибирское",
                "Усолье",
                "Уссурийск",
                "Усть-Джегута",
                "Усть-Илимск",
                "Усть-Катав",
                "Усть-Кут",
                "Усть-Лабинск",
                "Устюжна",
                "Уфа",
                "Ухта",
                "Учалы",
                "Уяр",
            ],
            "ф": ["Фатеж", "Феодосия", "Фурманов"],
            "х": [
                "Хабаровск",
                "Хадыженск",
                "Ханты-Мансийск",
                "Харабали",
                "Харовск",
                "Хасавюрт",
                "Хвалынск",
                "Хилок",
                "Химки",
                "Холм",
                "Холмск",
            ],
            "ц": ["Цивильск", "Цимлянск", "Циолковский"],
            "ч": [
                "Чадан",
                "Чайковский",
                "Чапаевск",
                "Чаплыгин",
                "Чебаркуль",
                "Чебоксары",
                "Чегем",
                "Чекалин",
                "Челябинск",
                "Чердынь",
                "Черемхово",
                "Черепаново",
                "Череповец",
                "Черкесск",
                "Чёрмоз",
                "Черногорск",
                "Чернушка",
                "Черняховск",
                "Чехов",
                "Чистополь",
                "Чита",
                "Чкаловск",
                "Чудово",
                "Чулым",
                "Чусовой",
                "Чухлома",
            ],
            "ш": [
                "Шагонар",
                "Шадринск",
                "Шали",
                "Шарья",
                "Шатура",
                "Шахты",
                "Шахунья",
                "Шацк",
                "Шелехов",
                "Шенкурск",
                "Шилка",
                "Шимановск",
                "Шиханы",
                "Шлиссельбург",
                "Шумерля",
                "Шумиха",
                "Шуя",
            ],
            "щ": ["Щербинка", "Щигры", "Щучье"],
            "ъ": None,
            "ы": None,
            "ь": None,
            "э": [
                "Электрогорск",
                "Электросталь",
                "Электроугли",
                "Элиста",
                "Энгельс",
                "Эртиль",
            ],
            "ю": [
                "Юбилейный",
                "Югорск",
                "Южа",
                "Южно-Сахалинск",
                "Южно-Сухокумск",
                "Южноуральск",
                "Юрга",
                "Юрьев-Польский",
                "Юрьевец",
                "Юрюзань",
                "Юхнов",
            ],
            "я": [
                "Ядрин",
                "Якутск",
                "Ялта",
                "Ялуторовск",
                "Янаул",
                "Яранск",
                "Ярославль",
                "Ясногорск",
                "Ясный",
                "Яхрома",
            ]
        }
	show_game_name(game_name)
        rules_or_game_choice_text = "Чтобы узнать правила, введите 1.\n\
Чтобы начать, введите 2.\n\
Ваш выбор: "
        rules_or_game_choice = ask_one_two(rules_or_game_choice_text)
        if rules_or_game_choice == 1:
	    show_cities_rules()

        mentioned = set()
        previous_city = None
        previous_last_letter = None
        last_letter = None
        turn = 1

	show_game_started(game_name)
        print("Это 1 кон")
        while True:
            city = input("Вводи название города: ")
            while True:
                if city in mentioned:
                    city = input("Такой город уже был. \nВводи название города: ")
                    continue
                elif turn > 1 and (city[0] != previous_last_letter):
                    # if not previous_last_letter means if it was not the first turn
                    city = input("Это слово не начинается на последнюю букву предыдущего города.\n\
Введите другое: ")
                    continue
                else:
                    break
            mentioned.add(city)
            last_letter = city[-1]
            if city[-1] in "ёъьы":
                last_letter = city[-2]
            for letter_list_key in abc.keys():
                if letter_list_key == last_letter and abc[letter_list_key]:
                    for possible_answer in abc[letter_list_key]:
                        if possible_answer not in mentioned:
                            mentioned.add(possible_answer)
                            previous_city = possible_answer
                            break
            if not previous_city:
                print(m_city_bot_lost)
                exit()
            print(f"А мой ответ - {previous_city}")
            previous_last_letter = previous_city[-2] if previous_city[-1] in "ёъьы" else previous_city[-1]
            previous_city = None
            turn += 1
            print(f"Это {turn} кон.")
    elif game_choice == "2":
        rules_or_game = check_one_two(input(ret_m_gametype_chosen('Числа')))
        if rules_or_game == "1":
	    show_numbers_rules()
        wins, loses, overall, balance, mentioned = 0, 0, 0, 0, set()
        your_num = print("Игра в 'Числа' началась.\n\
    Чтобы закончить, введите !\nТы начинаешь.\n\
    Вводи своё число.")
        your_num = input()
        while your_num != "!":
            try:
                your_num = int(your_num)
            except ValueError:
                your_num = input("Это не целое число, введите другое: ")
                continue
            if your_num in mentioned:
                your_num = input("Такое число уже было названо, введите другое.\nВведите число: ")
                continue
            elif not 1 <= int(your_num) <= 100:
                your_num = input("Твоё число не в ходит в диапазон от 1 до 100\nВведите число: ")
                continue
            mentioned.add(your_num)
            if len(mentioned) == 100:
                print("Чисел больше нет, игра завершилась.")
                your_num = "!"
                continue
            else:
                answer = randint(1, 100)
                while answer in mentioned:
                    answer = randint(1, 100)
                mentioned.add(answer)
            print(f"А мой ответ равен {answer}")
            overall += 1
            if answer > your_num:
                loses += 1
            elif answer < your_num:
                wins += 1
            else:
                balance += 1
            your_num = input("Вводи своё число: ")
        print(f"Пришло время подвести итоги.\n\
Пользователь выиграл: {wins}\n\
Пользователь проиграл: {loses}\n\
Баланс: {balance}\nВсего конов: {overall}\n")
        exit()
